...
- Instagram publish tasks now re-check that global cooldown again at worker start, so already-dispatched IG jobs are quietly re-queued instead of burning another Meta call during the cooldown window
...
- internal Instagram pacing retries (global cooldown / lane busy) now requeue silently without leaving a visible transient error message in the scheduled row
