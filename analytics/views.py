...
def _overview_reach(insights: list, platform: str):
    ...
    # Facebook page reach (page_impressions_unique) was deprecated/removed by Meta, so
    # snapshots carry no reliable page-level 7-day reach. Report None ("—") rather than
    # surface a misleading proxy.
    return None
...
def accounts_overview(request: HttpRequest) -> JsonResponse:
    ...
    for account in accounts:
        ...
        if followers is not None:
            ...
            # old. Sparse snapshots (e.g. one 60 days ago) would otherwise show a
            # months-long change mislabeled as 7 days.
            if prior and prior.fetched_at and prior.fetched_at >= now - timedelta(days=14):
        ...
        rows.append(
            {
                ...
                "engagement_7d": _overview_engagement(insights, posts, account.platform, week_ago),
